 ...
 * @param name -
 */
async function replaceESLintIgnore(destPath: string, name: string): Promise<void> {
    ...
    const eslintIgnoreRegex = /tsparticles\.preset\.template(\.bundle)?(\.min)?\.js/g;

    const replacedText = eslintIgnore.replace(eslintIgnoreRegex, `tsparticles.preset.${camelizedName}$1$2.js`);
 ...
 * @param name -
 */
async function replaceGitIgnore(destPath: string, name: string): Promise<void> {
    const gitIgnorePath = path.resolve(destPath, ".gitignore");
    const gitIgnore = await fs.readFile(gitIgnorePath, "utf-8");
    const camelizedName = camelize(camelize(name, "-"), " ");

    const gitIgnoreRegex = /tsparticles\.preset\.template(\.bundle)?(\.min)?\.js/g;

    const replacedText = gitIgnore.replace(gitIgnoreRegex, `tsparticles.preset.${camelizedName}$1$2.js`);
...
async function updateBundleFile(destPath: string, name: string): Promise<void> {
    ...
    const replacedText = bundle.replace(bundleRegex, `load${capitalizedName}Preset`);
...
async function updateIndexFile(destPath: string, name: string): Promise<void> {
    ...
    const replacedFuncText = index.replace(indexFunctionRegex, `load${capitalizedName}Preset`);

    const indexNameRegex = /"template"/g;

    const replacedNameText = replacedFuncText.replace(indexNameRegex, `"#${camelizedName}#"`);
...
async function updatePackageFile(destPath: string, name: string, description: string, repoUrl: string): Promise<void> {
    ...
    const pkg = await fs.readFile(pacakgePath, "utf-8");
    const camelizedName = camelize(camelize(name, "-"), " ");
    const dashedName = dash(camelizedName);
    const descriptionRegex = /"tsParticles template preset"/g;
    const replacedDescriptionText = pkg.replace(descriptionRegex, `"${description}"`);
    const fileRegex = /"tsparticles.preset.template.min.js"/g;
    const replacedFileText = replacedDescriptionText.replace(fileRegex, `"tsparticles.preset.${camelizedName}.min.js"`);

    const privateRegex = /"private": true/g;
    const replacedPrivateText = replacedFileText.replace(privateRegex, "");

    const nameRegex = /"tsparticles-preset-template"/g;
    const nameReplacedText = replacedPrivateText.replace(nameRegex, `"tsparticles-preset-${dashedName}"`);

    const repoUrlRegex = /"url": "git\+https:\/\/github\.com\/tsparticles\/preset-template\.git"/g;
    const repoUrlReplacedText = nameReplacedText.replace(repoUrlRegex, `"url": "git+${repoUrl}"`);

    const issuesUrlRegex = /"url": "https:\/\/github\.com\/tsparticles\/preset-template\/issues"/g;
    const replacedText = repoUrlReplacedText.replace(issuesUrlRegex, `"url": "${repoUrl.replace(".git", "/issues")}"`);
...
async function updateReadmeFile(destPath: string, name: string, description: string, repoUrl: string): Promise<void> {
    ...
    const replacedDescriptionText = readme.replace(readmeDescriptionRegex, `tsParticles ${description} Preset`);
    ...
    const readmeFileNameRegex = /tsparticles\.preset\.template(\.bundle)?\.min\.js/g;
    ...
    const replacedUsageText = replacedMiniDescriptionText.replace(readmeUsageRegex, `preset: "${camelizedName}`);

    const sampleImageRegex =
        /!\[demo\]\(https:\/\/raw.githubusercontent.com\/tsparticles\/preset-template\/main\/images\/sample.png\)/g;
    const repoPath = repoUrl.includes("github.com")
        ? repoUrl.substring(repoUrl.indexOf("github.com/") + 11, repoUrl.indexOf(".git"))
        : "tsparticles/preset-template";
    const replacedText = replacedUsageText.replace(
        sampleImageRegex,
...
async function updateWebpackFile(destPath: string, name: string, description: string): Promise<void> {
    ...
    const replacedDescriptionText = webpack.replace(webpackDescriptionRegex, `tsParticles ${description} Preset`);

    const webpackEntryRegex = /"template(\.bundle)?"/g;

    const replacedNameText = replacedDescriptionText.replace(webpackEntryRegex, `"${camelizedName}$1"`);
 ...
 * @param repoUrl -
 */
export async function createJavaScriptTemplate(name: string, description: string, repoUrl: string): Promise<void> {
    const destPath = path.resolve("."),
        emptyPath = path.resolve(__dirname, "../..", "files", "empty-project", "js"),
        sourcePath = path.resolve(__dirname, "../..", "files", "create-preset", "js");
